IF EXISTS (
    SELECT * FROM sysobjects WHERE name = 'FK_IngredientsQuantity_Ingredient'
)

BEGIN
    ALTER TABLE [dbo].[IngredientsQuantity] 
        DROP CONSTRAINT FK_IngredientsQuantity_Ingredient;
END

IF EXISTS (
    SELECT * FROM sysobjects WHERE name = 'FK_IngredientsQuantity_Recipes'
)

BEGIN
    ALTER TABLE [dbo].[IngredientsQuantity] 
        DROP CONSTRAINT FK_IngredientsQuantity_Recipes;
END

IF EXISTS (
    SELECT * FROM sysobjects WHERE name = 'FK_Recipes_Categories'
)

BEGIN
    ALTER TABLE [dbo].[Recipes] 
        DROP CONSTRAINT FK_Recipes_Categories;
END
